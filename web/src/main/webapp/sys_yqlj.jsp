...
{
	location.href='<%=basePath%>/prevOpenYqlj.action?ljId='+strId;
}
function toDel(strId)
{
	var t1='<%=basePath%>/deleteYqljById.action?ljId='+strId;
	if (confirm('确实删除此链接吗？'))
	{
		location.href=t1;
	}
...
function toPage(strP)
{
	...
	t1='<%=basePath%>/sysmanaYqlj.action?pCur='+strP+'&limit=<%=intPageLine%>&start='+i;
	location.href=t1;
}
function toTurnPage()
{
...
function toNew()
{
	var t1='<%=basePath%>/sys_yqljNew.jsp';
	location.href=t1;
}
//-->
</script>
...
<td style='height:40px;'>
	...
	<tr>
	<td width=240 ><font color='#A9714E' size='5'>友情链接</font></td>
...
<td>
    ...
    <td valign="top" align='left'>
        ...
        <tr>
        <td align='center' width=20% style='font-face:黑体;font-size:12px;'>链接名称</td>
        <td align='center' width=40% style='font-face:黑体;font-size:12px;'>链接地址</td>
		...
		}else
		{
			...
			{
				Yqlj sp=(Yqlj)ls.get(intLmp);
				%>
				<tr style='height:40px;'>
				<td valign='middle' class='td_dash' ><a href="javascript:doEditDoc('<%=sp.getLjId()%>')"><%=sp.getLjName()%></a></td>
				<td valign='middle' class='td_dash' ><%=sp.getLjAddr()%></td>
				<%
				...
				%>
		   		...
		   		<input type='button' value='删除' onclick="toDel('<%=sp.getLjId()%>')" />
   				</td></tr>
		   		<%
			}
		}
		%>
	   		...
	   		<input type='button' value='新增链接' onclick="toNew()" />
