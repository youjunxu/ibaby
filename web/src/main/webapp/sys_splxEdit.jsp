...
SpBrand sp=(SpBrand)request.getAttribute("waitEdit_doc");
...
<td>
    ...
    <td valign="top" align='left'>
   	<s:form method="post" action='updateSpBrandA.action' name="form1" theme="simple" enctype="multipart/form-data" >
  	<input type='hidden' name='brandId' value='<%=sp.getBrandId()%>' />
  	<input type='hidden' name='spTypeName' value='<%=sp.getSpTypeName()%>' />
  	<input type='hidden' name='brandImg' value='' />
        ...
        <td align='left'>
			...
			if (lss.size()>0)
			{
				intTmp=sp.getSpTypeId();
				for (intLmp=0;intLmp<lss.size();intLmp++)
				{
					SpType tp=(SpType) lss.get(intLmp);
					if (tp.getSpTypeId().equals(intTmp))
					{
						out.println("<option value='"+tp.getSpTypeId()+"' selected>"+tp.getSpTypeName()+"</option>");
					}else
					{
        ...
        <td align='left'>
            <INPUT class=form value="<%=sp.getBrandName()%>"
		...
		<td align='left' >
		<INPUT class=form value="<%=sp.getDispXh()==null?"":sp.getDispXh()%>"
		...
		<td></td>
		<td >
		<font color=red>(备注：如果不上传，将采用上次上传的照片。宽度、高度分别不超过78、41。)</font>
		</td>
		</tr>
			<tr><td style='height:5px;' colspan=2 class='td_dash'>&nbsp;</td></tr>
	   		<tr><td style='height:10px;' colspan=2>&nbsp;</td></tr>
        <tr>
        <td align='right' width=20% ><font size=4>LOG展示：</font></td>
        <td align='left'>
        <%
       	if (sp.getBrandImg()!=null)
       	{        	
       	%>
		<img src='<%=basePath%>/graph/lmxx/<%=sp.getBrandImg()%>' width=78 height=41 />
		<%
		}
		%>
        </td>
