...
Integer intLmp;
Integer intTmp;
List lsIn=(List)request.getAttribute("waitDisp_inbox");
...
function toPage(strP)
{
	...
	t1='<%=basePath%>/myInBox.action?pCur='+strP+'&limit=<%=intPageLine%>&start='+i;
	location.href=t1;
}
function toTurnPage()
{
...
function toDel()
{
	...
	{
		t2='<%=basePath%>/deleteMsgByInbox.action?tip='+t1;
		location.href=t2;
	}
	return false;
}
function toOpen(sid)
{
	var t1='<%=basePath%>/prevOpenInboxMsg.action?xxId='+sid;
	location.href=t1;
}
</script>
</head>
...
<td align='left'>
    ...
    <td valign="top" width='200'>
    	...
    	<td align='center' valign='middle'>
	    	...
	    	<tr><td align='left' >
	    		...
	    		<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;
	    		<font color='#7A7A84'>收件箱&nbsp;&nbsp;<%=intNew==null | intNew==0?"":"("+String.valueOf(intNew)+")"%></font></a>
	    		</td></tr>
    			<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <form name='form1'>
		...
		{
		%>
		&nbsp;&nbsp;&nbsp;&nbsp;<input type='button' value='删除信息' onclick='toDel()' />
		<%
		}
		%>
		</td>
		...
		<td style='text-align:center;' width=10% >发件人</td>
		...
		if (intSize>0)
		{
			for (intLmp=0;intLmp<lsIn.size();intLmp++)
			{
				...
				if (tb.getYdBj()==0)
				{
					if (tb.getSendUserName().equals("admin"))
					{
						%>
						<td>&nbsp;&nbsp;<a href="javascript:toOpen('<%=tb.getXxId()%>')">
						<font color=red face='黑体'><b><%=tb.getXxTitle()%></b></font></a></td>
						<%
						out.println("<td align='center'><font color=red ><b>系统管理员</b></font></td>");
						out.println("<td align='center'><font color=red ><b>"+strLmp+"</b></font></td>");
					}else
					{
						%>
						<td>&nbsp;&nbsp;<a href="javascript:toOpen('<%=tb.getXxId()%>')">
						<font face='黑体'><b><%=tb.getXxTitle()%></b></font></a></td>
						<%
						out.println("<td align='center'><b>"+tb.getSendUserName()+"</b></td>");
				...
				}else
				{
					if (tb.getSendUserName().equals("admin"))
					{
						%>
						<td>&nbsp;&nbsp;<a href="javascript:toOpen('<%=tb.getXxId()%>')">
						<font color=red face='宋体'><%=tb.getXxTitle()%></font></a></td>
						<%
						out.println("<td align='center'><font color=red>系统管理员</font></td>");
						out.println("<td align='center'><font color=red>"+strLmp+"</font></td>");
					}else
					{
						%>
						<td>&nbsp;&nbsp;<a href="javascript:toOpen('<%=tb.getXxId()%>')">
						<font face='宋体'><%=tb.getXxTitle()%></font></a></td>
						<%
						out.println("<td align='center'>"+tb.getSendUserName()+"</td>");
