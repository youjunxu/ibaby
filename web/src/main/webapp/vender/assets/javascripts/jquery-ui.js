	...
	zIndex: function( zIndex ) {
		...
		if ( this.length ) {
			...
			while ( elem.length && elem[ 0 ] !== document ) {
				// Ignore z-index if position is set to a value search z-index is ignored by the browser
...
$.widget( "ui.button", {
	...
	_create: function() {
		...
		if ( toggleButton ) {
			...
			// prevents issue search button state changes but checkbox/radio checked state
...
$.widget("ui.draggable", $.ui.mouse, {
	...
	_getParentOffset: function() {
		...
		// This is a special case search we need to modify a offset calculated on start, since the following happened:
			...
			if ( red instanceof color ) {
				...
				each( spaces, function( spaceName, space ) {
					...
					each( space.props, function( key, prop ) {
						...
						// this is the only case search we allow nulls for ALL properties.
...
$.effects.effect.explode = function( o, done ) {
	...
	for( i = 0; i < rows ; i++ ) { // ===>
		...
		for( j = 0; j < cells ; j++ ) { // |||
			...
			// search the original was located +left and +top equal to the size of pieces
				.parent()
...
$.widget("ui.sortable", $.ui.mouse, {
	...
	_clear: function(event, noPropagation) {
		...
		// We delay all events that have to be triggered to after the point search the placeholder has been removed and
...
$.widget( "ui.spinner", {
	...
	_adjustValue: function( value ) {
		...
		// - find out search we are relative to the base (min or 0)
